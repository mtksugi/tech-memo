...
## nvm
nodeのバージョンを管理する.  
プロジェクトごとに必要なバージョンが異なる（例: 古いプロジェクトは 18、Vite 6 などは 20 以上）ため、Mac 全体で 1 つに固定せず、プロジェクト単位で切り替える.

### Node と nvm の違い

| | 役割 |
|---|---|
| **Node.js** | JavaScript の実行環境（`node` / `npm`） |
| **nvm** | 複数バージョンを入れて切り替える管理ツール |

nvm を使う場合、Node は `nvm install` で入れる. Homebrew や公式インストーラと併用すると、どちらの `node` が使われるか混乱しやすい.

### 基本の仕組み

- **default** … 何も指定がないときのバージョン（`nvm alias default` で設定）
- **プロジェクト単位** … ルートの `.nvmrc` で指定
- **ターミナル単位** … `nvm use` はそのターミナルだけ切り替え（他のターミナルやフォルダには影響しない）
...
|----------|------|
| `nvm ls` | インストール済みの一覧 |
| `nvm install 20` | Node 20 をインストール |
| `nvm use 20` | このターミナルだけ Node 20 に切り替え |
| `nvm use` | カレントディレクトリの `.nvmrc` を読んで切り替え |
| `node -v` | 今使っているバージョンを確認 |
| `nvm current` | nvm 上の現在バージョンを確認 |
| `nvm alias default 18` | default を設定（**全体に影響するので注意**） |

`nvm use` は切り替え用. 確認だけなら `node -v` が簡単.
...
ルートに `.nvmrc` を置き、バージョン番号を書く.

```.nvmrc
v18.20.6
```

初回だけ `nvm install` でインストールし、開発のたびに `nvm use` で切り替える.
...
別プロジェクトに移れば default またはそのプロジェクトの `.nvmrc` が使われる. `.nvmrc` を置かないプロジェクトには影響しない.

### 他プロジェクトへの影響

| 影響しない | 影響する（注意） |
|------------|------------------|
| `.nvmrc` を置く | `nvm alias default` で default を変更 |
| `nvm install` | `brew upgrade node` などシステム全体の更新 |
| そのプロジェクトで `nvm use` | |

「このプロジェクトだけ 20」なら、default は触らず `.nvmrc` + `nvm use` で足りる.

### 他のバージョン管理ツール（参考）

fnm（軽量・自動切り替えしやすい）、volta（`package.json` に書く方式もある）、asdf（Node 以外も管理）などがある. どれか 1 つ使うのが普通.
